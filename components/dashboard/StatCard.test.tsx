...
describe('StatCard', () => {
  it('renders label, value, and unit', () => {
    render(
      <StatCard accent="blue" label="CO₂ this month" value={42.5} unit="kg" />,
    )
  ...
  it('renders icon badge when icon prop is provided', () => {
    const { container } = render(
      <StatCard accent="blue" label="Test" value={10} icon="leaf" />,
    )
  ...
  it('does not render icon badge when icon prop is absent', () => {
    const { container } = render(
      <StatCard accent="blue" label="Test" value={10} />,
    )
  ...
  it('does not render sub slot when sub is not provided', () => {
    const { container } = render(
      <StatCard accent="sage" label="Streak" value={7} />,
    )
  ...
  it('applies the design-token border class matching the accent prop', () => {
    const { container } = render(
      <StatCard accent="peach" label="Test" value={0} />,
    )
    const card = container.firstChild as HTMLElement
    expect(card.className).toContain('border-t-peach')
  })

  it('renders value without unit when unit is omitted', () => {
    render(<StatCard accent="slate" label="Days" value={30} />)
